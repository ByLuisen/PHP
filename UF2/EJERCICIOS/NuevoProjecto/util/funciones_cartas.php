...
function generarVistaHTML(array $cartas): void
{
    ...
    $contenido = strtr(file_get_contents('util/templates/indexTemplate.view.html'), ['{{contenido}}' => $contenidoCartas]);
...
function generarHTMLS(array $cartas): void
{
    ...
    foreach ($cartas as $nombre => $carta) {
        ...
        file_put_contents($ruta, strtr(file_get_contents('util/templates/letterTemplate.view.html'), ['{{contenido}}' => $carta]));
    }
}
...
function generarIndexHTML(array $nombres): void
{
    ...
    file_put_contents("index.html", strtr(file_get_contents('util/templates/indexTemplate.view.html'), ['{{contenido}}' => $contenido]));
}
